...
const openAIImageAPIKeyOnlyMessage = "OpenAI image endpoints only support API key accounts; OAuth accounts are not supported"

func (h *OpenAIGatewayHandler) ImagesGenerations(c *gin.Context) {
	h.handleImages(c, EndpointImagesGenerations, "handler.openai_gateway.images_generations")
}

func (h *OpenAIGatewayHandler) ImagesEdits(c *gin.Context) {
	h.handleImages(c, EndpointImagesEdits, "handler.openai_gateway.images_edits")
}

func (h *OpenAIGatewayHandler) handleImages(c *gin.Context, endpoint string, component string) {
	...
	if apiKey.Group == nil || apiKey.Group.Platform != service.PlatformOpenAI {
		h.errorResponse(c, http.StatusNotFound, "not_found_error", "This endpoint is not supported for this platform")
		return
	}
	...
	meta, err := service.ParseOpenAIImageRequestMeta(body, contentType)
	if err != nil {
	...
	reqLog = reqLog.With(
		...
		zap.Int("image_count", meta.ImageCount),
		zap.String("image_size", meta.ImageSize),
		zap.Bool("multipart", meta.Multipart),
	)
	...
	for {
		...
		if selectErr != nil {
			...
			case sawUnsupportedOAuth:
				h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", openAIImageAPIKeyOnlyMessage)
			case len(failedAccountIDs) == 0:
		...
		if account.Type != service.AccountTypeAPIKey {
			...
			switchCount++
			reqLog.Warn("openai_images.account_type_unsupported",
		...
		reqLog.Debug("openai_images.account_schedule_decision",
		...
		result, forwardErr := h.gatewayService.ForwardImage(c.Request.Context(), c, account, service.OpenAIImageForwardInput{
			Endpoint:      endpoint,
			Body:          body,
			ContentType:   contentType,
			Meta:          meta,
			ModelOverride: channelMappedModel(channelMapping),
		})
		...
		if forwardErr != nil {
			...
			if shouldLogOpenAIForwardFailureAsWarn(c, wroteFallback) {
				reqLog.Warn("openai_images.forward_failed", fields...)
				return
			}
			reqLog.Error("openai_images.forward_failed", fields...)
			return
		}
...
func channelMappedModel(mapping service.ChannelMappingResult) string {
	if mapping.Mapped {
