...
func (s *OpenAIGatewayService) shouldFailoverOpenAIUpstreamResponseForAccount(account *Account, statusCode int, upstreamMsg string, upstreamBody []byte) bool {
	...
	if statusCode != http.StatusNotFound || account == nil || account.FallbackAccountID == nil {
		return false
	}
	return *account.FallbackAccountID > 0
}
...
// deriveOpenAIReasoningEffortFromModelCandidates tries suffix inference in
// candidate order. OAuth/model mapping may remove the effort suffix from the
// final model, while the original client model still carries it.
...
func normalizeOpenAICodexCompactReasoningEffort(body []byte, effectiveModel string) ([]byte, bool, error) {
	...
	// Codex Ultra is orchestrated client-side as max, while the ChatGPT
	// compact endpoint currently accepts only through xhigh.
...
func isOpenAIGPT56Model(model string) bool {
	...
	return normalizeOpenAIGPT56ModelName(normalized) != ""
}
