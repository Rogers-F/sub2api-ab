...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(resp *http.Response, c *gin.Context) (OpenAIUsage, int, error) {
	...
	return usage, extractOpenAIImageResponseCountFromJSONBytes(body), nil
}
	...
	for {
		...
		if len(line) > 0 {
			...
			if data, ok := extractOpenAISSEDataLine(strings.TrimRight(string(line), "\r\n")); ok && data != "" && data != "[DONE]" {
				...
				if count := extractOpenAIImageResponseCountFromJSONBytes(dataBytes); count > imageCount {
					imageCount = count
				}
			}
		}
...
func extractOpenAIImageResponseCountFromJSONBytes(body []byte) int {
