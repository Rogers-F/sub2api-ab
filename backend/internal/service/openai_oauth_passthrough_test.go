...
func TestOpenAIGatewayService_OAuthPassthrough_CodexPresetInjectsMissingInstructions(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses/compact", bytes.NewReader(nil))
	c.Request.Header.Set("Content-Type", "application/json")
...
func TestOpenAIGatewayService_NonPassthrough_CodexPresetInjectsMissingInstructions(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.3-codex","stream":false,"input":[{"type":"text","text":"hi"}]}`)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
...
func TestOpenAIGatewayService_NonPassthrough_CodexPresetPreservesExistingInstructions(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.3-codex","instructions":"client instructions","stream":false,"input":[{"type":"text","text":"hi"}]}`)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
...
func TestOpenAIGatewayService_OAuthLegacy_StripImageGenerationSkipsCodexInjection(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Concurrency:    1,
		Credentials:    map[string]any{"access_token": "oauth-token", "chatgpt_account_id": "chatgpt-acc"},
		Extra:          map[string]any{"openai_strip_image_generation": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	}
	...
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	require.NotContains(t, string(upstream.lastBody), "image_generation")
	require.False(t, gjson.GetBytes(upstream.lastBody, "tools").Exists())
	require.False(t, gjson.GetBytes(upstream.lastBody, "tool_choice").Exists())
}

func TestOpenAIGatewayService_OpenAIPassthrough_StripImageGeneration(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.2","tools":[{"type":"web_search"},{"type":"image_generation","output_format":"png"}],"tool_choice":{"type":"image_generation"},"input":[{"type":"text","text":"hi"}]}`)
	...
	account := &Account{
		...
		Extra:          map[string]any{"openai_passthrough": true, "openai_strip_image_generation": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	}
