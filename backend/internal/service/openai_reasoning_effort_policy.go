...
// NormalizeMaxReasoningEffort canonicalizes a supported group policy value.
...
func NormalizeReasoningEffortMappings(platform string, raw []ReasoningEffortMapping) ([]ReasoningEffortMapping, error) {
	...
	for i, mapping := range raw {
		if utf8.RuneCountInString(mapping.From) > maxReasoningEffortValueLen ||
			utf8.RuneCountInString(mapping.To) > maxReasoningEffortValueLen {
...
func ApplyOpenAIReasoningEffortPolicy(body []byte, maxEffort string, mappings []ReasoningEffortMapping) ([]byte, bool) {
	...
	for _, path := range []string{"reasoning.effort", "reasoning_effort"} {
		...
		effective := mapReasoningEffort(original, mappings)
