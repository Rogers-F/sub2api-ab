...
const resetForm = () => {
  ...
  customErrorCodeInput.value = null
  interceptWarmupRequests.value = false
  nonStreamForceFailoverEnabled.value = false
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (openaiStripImageGenerationEnabled.value) {
    extra.openai_strip_image_generation = true
  } else {
    delete extra.openai_strip_image_generation
  }
...
const buildAnthropicExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (anthropicInvalidParamRectifierEnabled.value) {
    extra.anthropic_invalid_param_rectifier = true
  } else {
    delete extra.anthropic_invalid_param_rectifier
  }
...
const doCreateAccount = async (payload: CreateAccountRequest) => {
  const finalExtra: Record<string, unknown> = { ...(payload.extra || {}) }
  applyNonStreamForceFailover(finalExtra, nonStreamForceFailoverEnabled.value, 'create')
  const normalizedPayload: CreateAccountRequest = {
    ...payload,
    extra: Object.keys(finalExtra).length > 0 ? finalExtra : undefined
  }
