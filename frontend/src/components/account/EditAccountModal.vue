  ...
  >
    ...
    >
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.nonStreamForceFailover') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.nonStreamForceFailoverHint') }}
            </p>
          </div>
          <button
            type="button"
            @click="nonStreamForceFailoverEnabled = !nonStreamForceFailoverEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              nonStreamForceFailoverEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                nonStreamForceFailoverEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
      ...
      <!-- OpenAI image_generation stripping switch -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.stripImageGeneration') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.stripImageGenerationDesc') }}
            </p>
          </div>
          <button
            type="button"
            data-testid="openai-strip-image-generation-toggle"
            @click="openaiStripImageGenerationEnabled = !openaiStripImageGenerationEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openaiStripImageGenerationEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openaiStripImageGenerationEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.anthropic.invalidParamRectifier') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.anthropic.invalidParamRectifierDesc') }}
            </p>
          </div>
          <button
            type="button"
            @click="anthropicInvalidParamRectifierEnabled = !anthropicInvalidParamRectifierEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              anthropicInvalidParamRectifierEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                anthropicInvalidParamRectifierEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
import { applyNonStreamForceFailover } from '@/components/account/accountExtraBuilder'
...
const nonStreamForceFailoverEnabled = ref(false)
...
const openaiStripImageGenerationEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  nonStreamForceFailoverEnabled.value = extra?.non_stream_force_failover_enabled === true
  mixedScheduling.value = extra?.mixed_scheduling === true
  allowOverages.value = extra?.allow_overages === true

  // Load OpenAI passthrough toggle (OpenAI OAuth/API Key)
  openaiPassthroughEnabled.value = false
  openaiStripImageGenerationEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    ...
    openaiStripImageGenerationEnabled.value = extra?.openai_strip_image_generation === true
  ...
  if (newAccount.platform === 'anthropic' && newAccount.type === 'apikey') {
    ...
    anthropicInvalidParamRectifierEnabled.value = extra?.anthropic_invalid_param_rectifier === true
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  try {
    const finalExtra: Record<string, unknown> = {
      ...(((updatePayload.extra as Record<string, unknown> | undefined) || (props.account?.extra as Record<string, unknown> | undefined) || {}))
    }
    applyNonStreamForceFailover(finalExtra, nonStreamForceFailoverEnabled.value, 'edit')
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'anthropic' && props.account.type === 'apikey') {
      ...
      if (anthropicInvalidParamRectifierEnabled.value) {
        newExtra.anthropic_invalid_param_rectifier = true
      } else {
        delete newExtra.anthropic_invalid_param_rectifier
      }
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'apikey')) {
      ...
      if (openaiStripImageGenerationEnabled.value) {
        newExtra.openai_strip_image_generation = true
      } else {
        delete newExtra.openai_strip_image_generation
      }
